...
describe('DEV_BYPASS_AUTH', () => {
  ...
  it('accepts mock initData with non-hex hash when DEV_BYPASS_AUTH=true in development', () => {
    ...
    const result = validateInitData(initData, 'fake_bot_token')
    expect(result).not.toBeNull()
    expect(result!.id).toBe(123456789)
    expect(result!.first_name).toBe('Dev')
  })

  it('rejects mock initData when DEV_BYPASS_AUTH is not set', () => {
    process.env.NODE_ENV = 'development'
